...
export default function ResearchLabPage() {
  ...
  // When entering viewer mode, ensure we land on the requested card index.
  useEffect(() => {
    if (viewMode !== "viewer") return;
    const idx = pendingOpenIndexRef.current;
    if (idx === null) return;
    pendingOpenIndexRef.current = null;
    // set index first; scrolling will be handled by the viewer scroll effect.
    setCurrentIndex(idx);
  }, [viewMode]);
            ...
            <div className="flex flex-col gap-3 mb-6 md:flex-row md:items-center md:justify-between">
              <div className="relative w-full md:max-w-lg">
                ...
                <input
                  ...
                  className="w-full py-3 pl-10 pr-4 border rounded-xl bg-white dark:bg-surface-dark border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50"
                />
              </div>
              {searchQuery && (
                <button
